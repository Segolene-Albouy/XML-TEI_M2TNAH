...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:n="www.example.com"
      ...
      <xsl:param name="keepGlobals">true</xsl:param>
      <!-- was false -->
      <!-- the document corpus -->
      ...
      <xsl:param name="enumerateType">true</xsl:param>
      <!-- was false -->
     ...
     elements and attributes used in the corpus -->
                  <xsl:variable name="stage1">
                        <stage1>
                              ...
                              <docs>
                                    <xsl:for-each-group select="key('AllTEI', 1)"
                                          group-by="local-name()">
	     ...
	     deleted, by seeing if they are used anywhere-->
                              <xsl:for-each select="$stage1/stage1/tei/classSpec[@type = 'atts']">
                                    <xsl:variable name="classatts">
                                          ...
                                          <xsl:for-each select="attDef">
                                                ...
                                                <xsl:choose>
                                                  <xsl:when
                                                  test="$keepGlobals = 'true' and $this = 'n'">
                                                  <keep/>
                                                  </xsl:when>
                                                  <xsl:when
                                                  test="$keepGlobals = 'true' and $this = 'rendition'">
                                                  <keep/>
                                                  </xsl:when>
                                                  <xsl:when
                                                  test="$keepGlobals = 'true' and $this = 'style'">
                                                  <keep/>
                                                  </xsl:when>
                                                  <xsl:when
                                                  test="$keepGlobals = 'true' and $this = 'xml:id'">
                                                  <keep/>
                                                  </xsl:when>
                                                  <xsl:when
                                                  test="$keepGlobals = 'true' and $this = 'xml:base'">
                                                  <keep/>
                                                  </xsl:when>
                                                  <xsl:when
                                                  test="$keepGlobals = 'true' and $this = 'xml:space'">
                                                  <keep/>
                                                  </xsl:when>
                                                  <xsl:when
                                                  test="$keepGlobals = 'true' and $this = 'xml:lang'">
                                                  <keep/>
                                                  </xsl:when>
                                                  <xsl:when
                                                  test="$keepGlobals = 'true' and $this = 'rend'">
                                                  <keep/>
                                                  </xsl:when>
                                                  <xsl:when test="$used = ''">
                              ...
                              <xsl:if test="$processNonTEI = 'true'">
                                    <xsl:for-each-group select="key('All', 1)"
                                          group-by="local-name()">
                                          <xsl:sort/>
                                          <xsl:choose>
                                                <xsl:when test="self::n:ROOT"/>
                                                <xsl:when
                                                  test="namespace-uri() = 'http://www.tei-c.org/ns/1.0'"/>
                                                <xsl:otherwise>
                                                  <!-- build new 'add' elementSpec -->
                                                  <elementSpec ident="{current-grouping-key()}"
                                                  mode="add" ns="{namespace-uri()}">
                                                  <xsl:text>&#10;</xsl:text>
                        ...
                        <TEI xml:lang="en">
                              <teiHeader>
                                    <fileDesc>
                                          ...
                                          <sourceDesc>
                                                <p>Derived from <ref target="{$defaultSource}">base
                                                  odd</ref> after an analysis of <xsl:value-of
                                                  select="$count"/> files in <xsl:value-of
                                                  select="$corpus"/></p>
                                          </sourceDesc>
                                    </fileDesc>
                              </teiHeader>
	    ...
	    -->
                                                ...
                                                <xsl:for-each-group select="$stage2/stage2/*"
                                                  group-by="@module">
                                                  <xsl:variable name="m"
                                                  ...
                                                  <xsl:value-of
                                                  select="current-group()[self::elementSpec][not(@mode = 'delete')]/@ident"
                                                  separator=" "/>
                                                  </xsl:variable>
                                                  <xsl:if test="string-length($keepList) gt 1">
                                                  <moduleRef key="{$m}" include="{$keepList}"/>
                                                  </xsl:if>
                                                  </xsl:otherwise>
                                                  </xsl:choose>
                                                  <xsl:for-each
